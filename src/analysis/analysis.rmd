...
table_data_science <- gather(skills_data_science, "Skill", "Count", 14:30) %>%  arrange( Count) 
...
table_data_analist <- gather(skills_data_analist, "Skill", "Count", 14:30) %>%  arrange( Count) 
#make a plot of the counts for each skill
...
```{r, echo = FALSE}
#get only skills and their counts from the entire skills dataframe
table_marketing_analist <- gather(skills_marketing_analist, "Skill", "Count", 14:30) %>%  arrange( Count) 
#make a plot of the counts for each skill
...
```{r, echo = FALSE}
#get only skills and their counts from the entire skills dataframe
table_marketeer <- gather(skills_marketeer,"Skill", "Count", 14:30) %>%  arrange( Count) 
#make a plot of the counts for each skill
...
avg_salary_data_science <- mean(data_scientist_salary$min_salary_ds)
avg_salary_data_analist <- mean(data_analist_salary$min_salary_da)
avg_salary_marketing_analist <- mean(marketing_analist_salary$min_salary_ma)
avg_salary_marketeer <- mean(marketeer_salary$min_salary_m)
...
# filter cities with over 30000 euro's average yearly salary, filtering for outliers
data_scientist_avgsalary  <- data_scientist_avgsalary %>%
  filter(data_scientist_avgsalary$average_salary > 40000 & average_salary < 300000)
...
data_analist_avgsalary <- data_analist_avgsalary  %>%
  filter(data_analist_avgsalary$average_salary > 40000 & average_salary < 300000)
...
marketing_analist_avgsalary <-   marketing_analist_avgsalary %>%
  filter(marketing_analist_avgsalary$average_salary > 40000 & average_salary < 300000)
...
marketeer_avgsalary <- marketeer_avgsalary %>%
  filter(marketeer_avgsalary$average_salary > 40000 & average_salary < 300000)
# build a plot of the locations with the highest average salary in the Netherlands
...
plot_salary <- ggplot(salary, aes(x= Location, y = Salary, fill=Job)) + geom_bar(stat='identity', position='dodge') + coord_flip()
plot_salary
```
