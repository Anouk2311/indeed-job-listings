...
After downloading and merging data, there are four datasets in total. One dataset per search term for the following search terms: Data Scientist, Data Analist, Marketing Analist and Marketeer. The following section will provide a short overview of how the datasets look like. 

### 3.1 This is what the data scientist dataset looks like
We can see that there are `r length(data_scientist$id)` job listings. It contains `r ncol(data_scientist)` columns.
...
We can see that there are `r length(data_analist$id)` job listings. It contains `r ncol(data_analist)` columns.
...
We can see that there are `r length(marketing_analist$id)` job listings. It contains `r ncol(marketing_analist)` columns.
```{r, echo = FALSE}
colnames(marketing_analist)
```

Here, a summary of the marketing analist can be observed.
...
We can see that there are `r length(marketeer$id)` job listings. It contains `r ncol(marketeer)` columns.
...
1. Skill keyword analysis: we analyze the frequency of occurrence for certain technical skills related to marketing analytics individuals who pursue such a job.

2. Location analysis: after knowing the right skills required for a future job, we provide a location analysis in which we investigate what are the top locations for the four jobs.
...
skills_data_science <- data_scientist %>%
  ...
  mutate('Power BI' = str_count(data_scientist$description, "Power BI")) %>%
...
### 4.1 Getting the skill counts for the most important skills in data scientist jobs
...
This is a plot of the core technical skills and their counts in the data scientist job listings. As we can see, there are four clear top skills to learn in order to become a data scientist. These include Python, SQL, R and machine learning, with each having counts over 300. Other important skills seem to be Java, Excel, Azure and AWS (Amazon Web Services), with each having counts over 100.
...
skills_data_analist <- data_analist %>%
  ...
  mutate('Power BI' = str_count(data_analist$description, "Power BI")) %>%
  ...
  mutate('Big Data' = str_count(data_analist$description, "big data" )) %>% 
  mutate('Machine Learning' = str_count(data_analist$description, "machine learning" )) %>%
...
Again similar results for the percentage analysis. However, the difference between SQL and Excel is declining, with both being almost equally important. Also the difference with number three, Python, seems slightly smaller than in the count analysis. Again, other important skills are R, Power BI and Tableau. 
...
skills_marketing_analist <- marketing_analist %>%
  ...
  mutate('Power BI' = str_count(marketing_analist$description, "Power BI")) %>%
...
skills_marketeer <- marketeer %>%
  ...
  mutate('Power BI' = str_count(marketeer$description, "Power BI")) %>%
...
### 5.1 Location frequency data scientist
...
For data analists, the picture is quite similar. Amsterdam is again the clear winner in terms of number of jobs offered. However, Utrecht seems relatively more important for data analists compared to data scientists. Rotterdam again is number 3, with Den Haag and Eindhoven completing the top 5 in terms of locations.
...
#filter out 2 wrong job ads in the data which showed job ads for warehouse jobs with a salary of over 2 million
...
#filter out 2 wrong job ads in the data which showed job ads for warehouse jobs with a salary of over 2 million
data_analist_salary <- data_analist_salary %>% filter(salary_good < 500000)
# discard the column X.1 as it is a duplicate of the index column 
...
#filter out 2 wrong job ads in the data which showed job ads for warehouse jobs with a salary of over 2 million
marketing_analist_salary <- marketing_analist_salary %>% filter(salary_good < 500000)
# discard the column X.1 as it is a duplicate of the index column 
...
#filter out 2 wrong job ads in the data which showed job ads for warehouse jobs with a salary of over 2 million
marketeer_salary <- marketeer_salary %>% filter(salary_good < 500000)
# discard the column X.1 as it is a duplicate of the index column 
...
Data Scientists on average have the highest salaries, earning around 50000 annually on average. The difference with Marketing Analysts and Data Analysts is small however, with both analyst jobs having a mean salary of well over 40000 per year. Marketeers seem to fall behind in terms of salary, earning less than 40000 per year. 

In the next section we analyze the top locations based on average salaries. Due to limited salary data, many locations have only a single job ad with salary data included, this number does not really represent an average. Therefore, we decided to only incorporate locations with a minimum of 3 job ads into the analysis.
...
plot_mean_salary <- plot(ggplot(df_salary, aes(x = job, y = salary)) +
  ...
  xlab("Job Title") +
  ylab("Salary") +
  ggtitle("Average salary per month") +
  theme_minimal()
```


### 6.2 Top locations in terms of salary for data scientist  

```{r, echo = FALSE, warning = FALSE, fig.align = 'left'}
data_scientist_salary <- transform(data_scientist_salary, freq.loc = ave(seq(nrow(data_scientist_salary)), location, FUN=length))

data_scientist_salary <- data_scientist_salary  %>%  filter(freq.loc > 2)
...
# filter cities with over 25000 euro's average yearly salary
data_scientist_avgsalary <- data_scientist_avgsalary  %>%
  filter(data_scientist_avgsalary$average_salary > 25000 )
...
### 6.3 Top locations in terms of salary for data analist
For Data Analysts, again Zaandam and Amsterdam are the cities with highest average salaries. Another attractive option is Apeldoorn. These top 3 cities all have average salaries close to or even over 60000 euro's per year. Quite more than the national average.  

```{r, echo = FALSE, warning = FALSE, fig.align = 'left'}
# average salaries per location for top locations for data analists
data_analist_salary <- transform(data_analist_salary, freq.loc = ave(seq(nrow(data_analist_salary)), location, FUN=length))

data_analist_salary <- data_analist_salary  %>%  filter(freq.loc > 2)
...
data_analist_avgsalary <- data_analist_avgsalary  %>%
  filter(data_analist_avgsalary$average_salary > 25000) 
...
### 6.4 Top locations in terms of salary for marketing analist
For Marketing Analysts, when looking for the highest salaries on average, the best places to find a job are Amsterdam, Almere, Den Haag and Utrecht.  

```{r, echo = FALSE, warning = FALSE, fig.align = 'left'}
marketing_analist_salary <- transform(marketing_analist_salary, freq.loc = ave(seq(nrow(marketing_analist_salary)), location, FUN=length))

marketing_analist_salary <- marketing_analist_salary  %>%  filter(freq.loc > 2)
...
# filter cities with over 25000 euro's average yearly salary, filter for outliers
marketing_analist_avgsalary <-   marketing_analist_avgsalary %>%
  filter(marketing_analist_avgsalary$average_salary > 25000 )
...
### 6.5 Top locations in terms of salary for marketeer
The highest paid jobs for marketeers generally are found in Zaandam and Schiedam, with Zaandam having salaries over 60000, which is nearly twice the average salary for marketeers. Besides these 2 cities, there are numerous other cities including Utrecht, Amsterdam, Rotterdam, Nijmegen and Eindhoven all pretty similar to each other in terms of salaries, coming close to 40000 euro average annual income.

```{r, echo = FALSE, warning = FALSE, fig.align = 'left'}
marketeer_salary <- transform(marketeer_salary, freq.loc = ave(seq(nrow(marketeer_salary)), location, FUN=length))


marketeer_salary <- marketeer_salary  %>%  filter(freq.loc > 2)
...
marketeer_avgsalary <- marketeer_avgsalary %>%
  filter(marketeer_avgsalary$average_salary > 25000 )
...
This plot shows that data scientists in general dominate salaries in each location, shortly followed by data analists and marketing analists. Marketeers in general lag quite behind in terms of salaries.  
