...
After downloading and merging the data, we ended up with four datasets in total. One dataset per search term for the following search terms: Data Scientist, Data Analist, Marketing Analist and Marketeer. The following section will provide a short overview of what the datasets look like.
...
Here can be observed a summary of the data scientist dataset.
...
Here can be observed a summary of the data analist dataset.
...
Here can be observed a summary of the marketing analist dataset.

```{r}
summary(marketing_analist)
```

### 3.4 This is what the marketeer dataset looks like:
...
Here can be observed a summary of the marketeer dataset.
...
table_data_science <- gather(skills_data_science, "skill", "count", 15:31) %>%
  arrange(count) 

# make a plot of the counts for each skill
count_plot_data_science <- ggplot(data = table_data_science, aes(x = count, y = skill)) +
...
percentage_plot_data_science <- ggplot(data = percentage_data_science,
...
table_data_analist <- gather(skills_data_analist, "skill", "count", 15:31) %>%
  arrange(count) 
...
```{r}
# get only skills and their counts from the entire skills dataframe
table_marketing_analist <- gather(skills_marketing_analist, "skill", "count", 15:31) %>%
  arrange(count) 
...
```{r}
# get only skills and their counts from the entire skills dataframe
table_marketeer <- gather(skills_marketeer,"skill", "count", 15:31) %>%
  arrange(count)
...
data_scientist_salary <- data_scientist %>% 
  ...
  separate(salary1, into = c('salary1', 'salary2'), convert = TRUE, extra = 'drop') %>%
...
data_scientist_salary_plot <- ggplot(data_scientist_avgsalary,
  ...
  xlab("Average salary") +
  ylab("Location") +
  ggtitle("Average salery per location for Data Scientists") +
  theme_minimal()

data_scientist_salary_plot
```

### 6.3 top locations in terms of salary for data analists
...
data_analist_salary_plot <- ggplot(data_analist_avgsalary,
  ...
  xlab("Average salary") +
  ylab("Location") +
  ggtitle("Average salery per location for Data Analists") +
  theme_minimal()  

data_analist_salary_plot
```

### 6.4 top locations in terms of salary for marketing analists
...
marketing_analist_salary_plot <- ggplot(marketing_analist_avgsalary,
  ...
  xlab("Average salary") +
  ylab("Location") +
  ggtitle("Average salery per location for Marketing Analists") +
  theme_minimal()
...
marketeer_salary_plot <- ggplot(marketeer_avgsalary,
  ...
  xlab("Average salary") +
  ylab("Location") +
  ggtitle("Average salery per location for Marketeers") +
  theme_minimal()
...
For salary, data scientist seem to be in the best position, with on average the highest salaries compared to marketeers, data analists and marketing analists. 
