...
This dataset (use name of dataset) was created to provide current Marketing Analytics students with information about the skills that are in demand in today's job market. Currently, students trust that their university will teach them the skills that are going to be necessary in their future careers. However, are the teached skills in line with the skills that are really required nowadays? To answer this question, job listings from Indeed are being analyzed. Indeed is a worldwide employment website for job listings. Currently, Indeed is the market-leader within the Dutch labor market (do we have a source for this? Why indeed and not eg glassdoor?). We focused on job listings with the search terms 'Data Analist', 'Data Scientist', 'Marketing Analist' and 'Marketeer' with the location 'Nederland' to make the data as useful as possible for Marketing Analytics students. However, this data could also be relevant for students in related fields, as well as other people looking for a job in one of those disciplines. In addition, educational institutes can use this data to shape their course offerings.
...
This dataset was created by Anouk Heemskerk, Georgiana Huţanu, Renée Nieuwkoop and Alan Rijnders, who are students at Tilburg University. The creation of this dataset was part of the course Online Data Collection and Management instructed by dr. Hannes Datta.
...
The dataset contains .... job postings in total (Number of jobs we were able to scrape in the definitive dataset). Hereof, XX listings correspond to the search term 'Data Analist', XX to 'Data Scientist', 566 to 'Marketing Analist' and XX to 'Marketeer'.
...
The dataset contains all possible instances available on Indeed.com with the search term for 'Wat': 'Data Analist', and for 'Waar': 'Nederland'.
...
The data in the dataset consists of raw data in the form of unprocesssed texts. The data is directly scraped from Indeed and no data preprocessing has been conducted. For each job listing, the job listing id, title, company, location, posting date, summary, salary if applicable, url, full job description and the date and time the data was scraped were collected.
...
For some job listing, the salary indication is missing because this information was not always given. (do we miss more? eg are there job listings without full description?)
...
All data that is collected is derived from a publicly available website and does not contain any personal and/or confidential data. All companies that make use of Indeed to publish their job listings, are consent with making it publicly available. People do not need an account or login to see the job listings on Indeed. Therefore, the data is not considered confidential.
...
The dataset only contains textual information of job listings. This data is not expected to be offensive or insulting in any possible way.

#### *2.12 Does the dataset relate to people? If not, you may skip the remaining questions in this section.*

The dataset does not relate to individual people.
...
The four students who created this dataset and their professor Hannes Datta were involved in the data collection process. None of them were compensated.
