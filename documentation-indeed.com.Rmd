...
To collect the data we used Python and multiple Python packages to create a webscraper. The webscraper was manually curated by the 4 students named earlier.

*Detailed explanation of what we did. use 3 alternatives. Also where it is saved.*\

Furthermore, we used a Github repository to keep track of our project.
...
*summary statistics scrape time*
...
Yes preprocessing has taken place in order to merge the description and the listing CSV files together, by id, into 1 CSV per jobtitle. The preprocessing has been done to improve the structure and the accessibility of the datasets.
